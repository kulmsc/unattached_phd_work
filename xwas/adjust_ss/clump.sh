...
cat all_specs/clump_param_specs | tail -n +2 | while read spec;do
  ...
  if [ ! -e ~/athena/xwas/mod_sets/${author}/${low_author}.${chr}.clump.${i}.ss ]; then
    ...
    if [ -f ${d}/out.clumped ]; then
      ...
      fgrep -w -f ${d}/done_rsids temp_files/ss.${low_author}.${chr} > ~/athena/xwas/mod_sets/${author}/${low_author}.${chr}.clump.${i}.ss
    fi
  fi
