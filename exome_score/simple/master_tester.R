...
allor <- rep(0, ncol(test_exo_score))
for(i in 1:ncol(test_exo_score)){
 allp[i] <- wilcox.test(test_exo_score[df_test$pheno == 0, i], test_exo_score[df_test$pheno == 1, i])$p.value 
 tempmat <- matrix(c(sum(df_test$pheno[test_exo_score[,i] > quantile(test_exo_score[,i], 0.95)]),
                     sum(df_test$pheno[test_exo_score[,i] > quantile(test_exo_score[,i], 0.95)] == 0),
                     sum(df_test$pheno[test_exo_score[,i] < quantile(test_exo_score[,i], 0.05)]),
                     sum(df_test$pheno[test_exo_score[,i] < quantile(test_exo_score[,i], 0.05)] == 0)), nrow = 2)
 allor[i] <- fisher.test(tempmat)$est
}
