...
all_use_eids <- list()
for(kk in 1:3){
  all_use_eids[[kk]] <- read.table(paste0("eid_gwas_", kk+5), stringsAsFactors=F)
}
...
if(REAL_age_ind == 6){
  simple_covar_file <- simple_covar_file[simple_covar_file[,1] %in% bmi_data[bmi_data[,2] <= 25,1],]
  pheno_df_simple <- pheno_df_simple[pheno_df_simple[,1] %in% bmi_data[bmi_data[,2] <= 25,1],]
} else if(REAL_age_ind == 7){
  simple_covar_file <- simple_covar_file[simple_covar_file[,1] %in% bmi_data[bmi_data[,2] > 25 & bmi_data[,2] < 30,1],]
  pheno_df_simple <- pheno_df_simple[pheno_df_simple[,1] %in% bmi_data[bmi_data[,2] > 25 & bmi_data[,2] < 30,1],]
} else if(REAL_age_ind == 8){
  simple_covar_file <- simple_covar_file[simple_covar_file[,1] %in% bmi_data[bmi_data[,2] >= 30,1],]
  pheno_df_simple <- pheno_df_simple[pheno_df_simple[,1] %in% bmi_data[bmi_data[,2] >= 30,1],]
}
