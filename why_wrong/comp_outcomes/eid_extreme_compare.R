...
big_data <- big_data[big_data$eid %in% c(ppl_1, ppl_2),]

run_df <- data.frame(eid = big_data$eid)
run_df$y <- 0
run_df$y[run_df$eid %in% ppl_2] <- 1
run_df$age <- big_data$SURVEY_X31.0.0

small_data <- big_data[big_data$eid %in% c(ppl_1, ppl_2),]
small_data <- small_data[,colnames(small_data) != "SURVEY_X31.0.0"]
