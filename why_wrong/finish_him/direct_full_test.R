...
#batch_type <- args[2]
case_type <- 3
#case_type <- as.numeric(args[2])
print(paste(author, case_type))
...
if(batch_type == "time"){
  train_time <- readRDS("../batch_indivs/out_batch/timeline_groups.train.RDS")
  test_time <- readRDS("../batch_indivs/out_batch/timeline_groups.test.RDS")

  better_train_df <- train_df[!(train_df$eid %in% train_time[[case_type]]),]
  better_test_df <- test_df[!(test_df$eid %in% test_time[[case_type]]),]

} else if(batch_type == "cc"){
  train_case <- readRDS(paste0("../batch_indivs/out_batch/case_groups.train.", author, ".RDS"))
  test_case <- readRDS(paste0("../batch_indivs/out_batch/case_groups.test.", author, ".RDS"))
  train_control <- readRDS(paste0("../batch_indivs/out_batch/control_groups.train.", author, ".RDS"))
  test_control <- readRDS(paste0("../batch_indivs/out_batch/control_groups.test.", author, ".RDS"))


  better_train_df <- data.frame(train_df)
  better_test_df <- data.frame(test_df)
  better_train_df$pheno[better_train_df$eid %in% train_case[[case_type]]] <- 1
  better_test_df$pheno[better_test_df$eid %in% test_case[[case_type]]] <- 1
  better_train_df$pheno[better_train_df$eid %in% train_control[[case_type]]] <- 0
  better_test_df$pheno[better_test_df$eid %in% test_control[[case_type]]] <- 0

} else if(batch_type == "resid"){
  train_resid <- readRDS(paste0("../batch_indivs/out_batch/resid_groups.train.", author, ".RDS"))
  test_resid <- readRDS(paste0("../batch_indivs/out_batch/resid_groups.test.", author, ".RDS"))

  better_train_df <- train_df[!(train_df$eid %in% train_resid[[case_type]]),]
  better_test_df <- test_df[!(test_df$eid %in% test_resid[[case_type]]),]

} else if(batch_type == "roll"){

  train_control <- readRDS(paste0("../batch_indivs/out_batch/roll_groups.train.", author, ".RDS"))
  test_control <- readRDS(paste0("../batch_indivs/out_batch/roll_groups.test.", author, ".RDS"))
...
exit()
print(nrow(train_df))
print(nrow(better_train_df))
print("")
print(nrow(test_df))
print(nrow(better_test_df))
...
get_over_here <- function(df_train, df_test, df_type){
  ...
  all_base_holder <- list("auc" = base_auc, "or" = all_base_odds_ratio)
  all_score_holder <- list("auc" = score_auc, "or" = all_score_odds_ratio)
  ...
  final_holder <- list("base" = all_base_holder, "score" = all_score_holder)
  #return(final_holder)
