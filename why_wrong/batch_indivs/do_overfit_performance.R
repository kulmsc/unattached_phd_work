...
big_switch <- "explicit" #explicit, check_pheno, all_remove
...
run_cv <- function(input_df, remove_eids, the_action, input_folds = 3, input_repeats = 10){
  ...
  for(i in 1:input_repeats){
    ...
    for(j in 1:input_folds){
      ...
      if(length(unique(input_df$pheno[input_df$eid %in% test_group])) != 2 |
        ...
        auc <- roc(input_df[input_df$eid %in% test_group,]$pheno ~ preds, quiet = TRUE)$auc
...
#base_auc <- mean(run_cv(train_df, 1, "nothing"))
#saveRDS(base_auc, paste0("res/base_auc.", author, ".RDS"))
base_auc <- readRDS(paste0("res/base_auc.", author, ".RDS"))
...
step_through <- function(all_groups, start_remove){
  ...
  for(i in 1:length(all_groups)){
    for(j in 1:length(all_groups[[i]])){
      cvres <- run_cv(train_df, c(all_groups[[i]][[j]], start_remove ), all_actions[i])
...
saveRDS(list(best_train_remove, best_test_remove, all_auc, all_remove_group, all_remove_index), paste0("res/togo.all_around.", author, ".", big_switch, ".RDS"))

saveRDS(list(mean_train_remove, mean_test_remove, mean_auc, back_list_big[[1]][[4]], back_list_big[[1]][[5]]), paste0("res/togo.mean.", author, ".", big_switch, ".RDS"))

saveRDS(list(min_train_remove, min_test_remove, min_auc, back_list_big[[2]][[4]], back_list_big[[2]][[5]]), paste0("res/togo.min.", author, ".", big_switch, ".RDS"))

saveRDS(list(count_train_remove, count_test_remove, count_auc, back_list_big[[3]][[4]], back_list_big[[3]][[5]]), paste0("res/togo.count.", author, ".", big_switch, ".RDS"))
