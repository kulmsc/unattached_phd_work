...
date_attend <- read.csv("date_attend.csv", stringsAsFactors=F, header=T)
date_attend[,2] <- as.Date(date_attend[,2])
...
end_date[censor[,2] != ""] <- censor[censor[,2] != "", 2] #lost to follow up
end_date[death[,5] != ""] <- death[death[,5] != "", 5]
final_rec_date <- as.character(end_date)
end_date[!is.na(dates)] <- dates[!is.na(dates)]


is_death_date <- rep(0, nrow(scores))
is_death_date[death[,5] == end_date] <- 1
surv_df <- data.frame(time = as.numeric(as.Date(end_date) - as.Date(start_date)), pheno, is_death_date, covars, score = scores[,1], start_date, end_date, final_rec_date)
df <- data.frame(pheno, covars[,-1], score = scores[,1])


#need to remove people that had diagnosis before accepted start of the study
end_date <- end_date[surv_df$time > 0]
...
date_attend <- date_attend[date_attend[,1] %in% eid,]
date_attend <- date_attend[order(date_attend[,1])[rank(eid)],]

surv_df$date_attend <- date_attend[,2]

covars <- covars[end_date > date_attend[,2],]
pheno <- pheno[end_date > date_attend[,2]]
eid <- eid[end_date > date_attend[,2]]
df <- df[end_date > date_attend[,2],]
surv_df <- surv_df[end_date > date_attend[,2],]
end_date <- end_date[end_date > date_attend[,2]]
...
event_type[surv_df$pheno == 1] <- "diagnosis" #you can be diagnosed on same date as death, in which case the primary event is the diagnosis
surv_df$event_type <- as.factor(event_type)
surv_df$eid <- eid

birthday <- read.csv("date_of_birth.csv", stringsAsFactors=F, header=T)
birthday$final <- as.Date(paste0("15-", birthday[,3], "-", birthday[,2]), "%d-%m-%Y")
birthday <- birthday[birthday$eid %in% surv_df$eid,]
birthday <- birthday[order(birthday$eid)[rank(surv_df$eid)],]
surv_df$dob <- birthday$final


saveRDS(surv_df, paste0("surv_data/survdf.", tolower(author), ".RDS"))
...
#wrong_list <- list(123456, surv_df$eid[surv_df$pheno == 1][end_date[surv_df$pheno == 1] > as.Date("2019-06-01")],
#     surv_df$eid[surv_df$pheno == 1][end_date[surv_df$pheno == 1] > as.Date("2019-01-01")],
#     surv_df$eid[surv_df$pheno == 1][end_date[surv_df$pheno == 1] > as.Date("2018-06-01")],
#     surv_df$eid[surv_df$pheno == 1][end_date[surv_df$pheno == 1] > as.Date("2018-01-01")])

#saveRDS(wrong_list, paste0("out_batch/roll_groups.train.", tolower(author), ".RDS"))
#saveRDS(wrong_list, paste0("out_batch/roll_groups.test.", tolower(author), ".RDS"))
